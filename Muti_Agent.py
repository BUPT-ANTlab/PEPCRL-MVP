...
class Muti_agent():
    def __init__(self, params):
        ...
        for pursuer_id in self.params["pursuer_ids"]:
            self.agent_list[pursuer_id]=DQN(self.params,pursuer_id)
        if self.train_model=="evaluate":
            num_param=self.agent_list[self.params["pursuer_ids"][0]].get_param().shape[-1]
    ...
    def process_state(self,state):
        ...
        for pursuer_id in self.params["pursuer_ids"]:
            ...
            if "target" in state:
                ...
                target_code = self.agent_list[pursuer_id].DQN_Net.select_target(ego_pos_tensor,
    ...
    def select_action(self,pro_state, pro_all_states):
        ...
        for pursuer_id in self.params["pursuer_ids"]:
            ...
            action,action_prob=self.agent_list[pursuer_id].select_action(ego_state)
    ...
    def train_agents(self):
        ...
        for pursuer_id in self.params["pursuer_ids"]:
            ...
            if self.train_model=="evaluate" :
                ...
                self.prioritization_net_buffer.store_input_exp(pursuer_id,chosen_exp)
                w=math.pow(len(probs)*exp_prob,self.lamda)/max(np.power(len(probs)*probs,self.lamda)) #Importance Sampling
    ...
    def select_train_set(self, net_param):
        ...
        return self.replay_buffer.memory_pool[exp_index], chosen_exp,probs[exp_index],probs
